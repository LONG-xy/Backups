...
function IndustryOverp(IndustryThis) {
    ...
    if ($('#IndustryMoreDiv').length == 0) {
        ...
        Context += '<img src="sysImg/小啾.png" style="top:4px; left:110px; position:absolute;" />';
...
function SenceOverp(SenceThis) {
    ...
    if ($('#SenceMoreDiv').length == 0) {
        var Context = '<section id="SenceMoreDiv" onMouseOver="SenceOver()" onMouseOut="SenceOut()" style="width:230px; position:absolute; left:' + parseInt(y) + 'px; top:' + parseInt(x) + 'px;  padding-top:10px; padding-bottom:10px; z-index:100; overflow:hidden; text-overflow:clip; overflow:hidden;" class="bennTitleMore">';
        Context += '<img src="sysImg/小啾.png" style="top:4px; left:110px; position:absolute;" />';
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status != "-1") {
                ...
                $.each(dataobj.root, function (i, item) {
                    GetCount = +i;
                    var Html = '<section onclick="ClickTemp(\'' + item.temp_code + '\')" class="tempDiv" onMouseOver="tempMouseOver(this)" onMouseOut="tempMouseOut(this)">' +
                                    '<section style="height:176px; position:relative; margin-left:2px; margin-top:2px;">' +
                                        '<img src="' + item.cover + '" style="width:176px; height:176px; border-radius:5px 5px 0px 0px;">' +
                                        (parseInt(item.Money) > 0 ? '<div class="tempTopTitleQ"><font class="tempFont" style="font-size:8px">￥' + item.Money + '</font></div>' : '<div class="tempTopTitle"><font class="tempFont">免</font></div>') +
                                    '</section>' +
                                   ...
                                   '</section>' +
                                    '<div id="qr' + item.temp_code + '" style="display:none; position:absolute;top:0px;left:0px; z-index:4;">' +

                                    '</div>' +
                                '</section>';
                    ContentDiv.append(Html);

                    var qrcode = new QRCode(document.getElementById("qr" + item.temp_code), {
                        width: 180,//设置宽高
                        height: 180
                    });
                    qrcode.makeCode('http://' + window.location.host + '/senceCreate/view.html?c=view&id=' + item.scene_code + '&preview=preview');
                });
                if (GetCount < 23) {
                    $('#GetMore').hide();
                }
...
function ClickTemp(temp_code) {
    ...
    EjectIdent('TempDetailed.html?temp_code=' + temp_code, '您要使用的场景', 760, 650);
}
