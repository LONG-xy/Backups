...
namespace web.Tools
{
    public static class f
    {
        ...
        public static void SendEmail(string userEmail, string code, int t)
        {
            ...
            if (t == 0)
            {
                SendEmail(userEmail, "SHOW注册验证", "http://" + url + "/checkEmail.aspx?code=" + code + "&un=" + userEmail);
            }
            else
            {
                SendEmail(userEmail, "SHOW找回密码", "http://" + url + "/PwdEmail.aspx?code=" + code + "&un=" + userEmail);
            }
        }
        /// <summary>
        /// 发送邮件
